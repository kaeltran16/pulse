...
- **5b** Spec + plan complete 2026-04-27 — design at [`../2026-04-27-sp5b-pulse-worker-design.md`](../2026-04-27-sp5b-pulse-worker-design.md), plan at [`../../plans/2026-04-27-sp5b-pulse-worker-plan.md`](../../plans/2026-04-27-sp5b-pulse-worker-plan.md). Locks: `imapflow` + `mailparser`, AES-256-GCM via Node `crypto` with separate `PULSE_IMAP_ENCRYPTION_KEY` env var, recurring heuristic ±10% same-currency, 5-min default poll with exponential backoff capped at 1h, in-process `parseEntry()` lib (extracted from `/parse` route). Implementation pending.
- **5c** Not started.
- **5d** Not started.
- **5e** Not started.
- **5f** Not started.
