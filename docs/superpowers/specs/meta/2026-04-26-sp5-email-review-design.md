...
- **5f** ✅ Code complete 2026-04-29 — StreakPill component on `app/(tabs)/rituals/index.tsx` rows (hidden when streak ≤ 1, themed by ritual color); new `app/celebration.tsx` modal-styled route fired on app foreground when any per-ritual high-water-mark is broken (coalesces multiple breaks into one modal showing the highest-streak winner; bumps all broken HWMs in one pass); new `app/close-out.tsx` full-screen route fired when local hour ≥ 21 and distinct-rituals-today < `goals.dailyRitualTarget` and not already dismissed (renders checklist via `useLiveQuery`, tapping rows calls `toggleRitualToday`, "Good night" gate uses daily goal, both back-out and "Good night" persist a same-day dismissal). Schema delta: new `ritual_streak_high_water` and `dismissed_close_outs` tables (migration `0006_acoustic_iron_lad.sql`). New iOS query modules `lib/db/queries/streakHighWater.ts` and `lib/db/queries/closeOutDismissals.ts`. New `lib/sync/foregroundChecks.ts` orchestrator (re-entrance guard mirrors `syncNow.ts`) wired alongside `syncNow` in `app/_layout.tsx`'s AppState handler. `toggleRitualToday` extended to bump HWM on the insert path. `<PalComposer />` gains an optional `prefill?: string` prop (only consumer is Close-Out). New `lib/sync/nextMilestone.ts` pure helper (ladder 7/14/30/60/100/365). 45 new iOS tests (10 streakHighWater + 5 closeOutDismissals + 15 nextMilestone + 3 toggleRitualToday HWM + 12 foregroundChecks); iOS suite now 435 tests (was 390); backend unchanged at 226; root tsc baseline preserved at 28; backend tsc baseline preserved at 0. No backend changes. iPhone Expo Go visual verification + web smoke carry over to the SP5-wide deferred pass.
- **5g** ✅ Code complete 2026-04-30 — period-aware `/review` route accepts a discriminated-union `{ period: 'weekly' | 'monthly', periodKey, aggregates, signals }` request and returns LLM-authored prose (`hero`, `patterns[]` tagged by signal key, optional `oneThingToTry`); branched prompt builder produces a one-sentence weekly hero or 2-3 sentence monthly narrative and only emits patterns for non-null signals (route enforces both period-mismatch and stray-signal as 502). New iOS modules: `lib/db/queries/reviewAggregates.ts` (period bounds incl. ISO-week, `lastCompletedPeriodKey`, `computeReviewAggregates` over local SQLite, `computeReviewSignals` for topSpendDay / ritualVsNonRitual / bestStreak / underBudget, `isPeriodEmpty`); `lib/db/queries/generatedReviews.ts` (per-period cache get/put/clear); `lib/sync/reviewClient.ts` (postReview HTTP wrapper using existing pal/errors taxonomy). Schema delta: new `generated_reviews` table (migration `0007_early_expediter.sql`). New iOS routes `app/reviews/{weekly,monthly}.tsx` + `_layout.tsx` thin wrappers around shared `app/components/reviews/ReviewScreen.tsx` (chevron history, ±12 cap, cache-first orchestration, retry card on Pal failure, regenerate button with 60s cooldown, in-screen `<PalComposer prefill>` for "Ask Pal more"). New presentational components: ThreeStatSummary, HeroCard, PatternsList, OneThingToTry, ByTheNumbers, ReviewEmptyState, ReviewRetryCard. You-tab "Weekly review" / "Monthly review" rows flipped on. Breaking `lib/api-types.ts` change: `ReviewRequest` / `ReviewResponse` shape replaced wholesale; legacy `WorkoutAggregate` / `FoodAggregate` / `SpendAggregate` / `RitualAggregate` types removed. ~36 new iOS tests (22 reviewAggregates + 4 generatedReviews + 6 reviewClient + 4 ReviewScreen orchestration); iOS suite now 471 tests (was 435); ~17 new backend tests (10 reviewSchema + 3 reviewPrompt + 9 review integration replacing 2 legacy); backend suite now 243 tests (was 226 net of removed legacy); root tsc baseline now 6 (was 28 — improved by removing the legacy aggregate types and their dead references); backend tsc baseline preserved at 0. iPhone Expo Go visual verification + web smoke carry over to the SP5-wide deferred pass. Live droplet smoke (gated on `OPENROUTER_API_KEY`) carries over from SP5b/SP5c.
