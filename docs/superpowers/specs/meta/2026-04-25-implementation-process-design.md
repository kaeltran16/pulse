...
| 5 | Backend v2 + iOS v3 — email + review | Code complete 2026-04-30 — meta-spec at [`meta/2026-04-26-sp5-email-review-design.md`](./2026-04-26-sp5-email-review-design.md). All seven sub-slices code complete: 5a (backend data store, 2026-04-27 — Docker cutover to `/opt/pulse/` compose stack), 5b (`pulse-worker` service, 2026-04-27 — IMAP polling + AES-256-GCM credential encryption + recurring-detection heuristic), 5c (HTTP sync routes + iOS sync client, 2026-04-27 — `"sync"` JWT scope + cursor pagination + `lib/sync/` orchestrator), 5d (Email Sync flow + Subscriptions + You-tab settings hub, 2026-04-27), 5e (Rituals tab + Builder + `/suggest-rituals` + `/nudge-today`, 2026-04-28 — adds `expo-notifications`), 5f (Streak surface + Celebration + Evening Close-Out, 2026-04-29), 5g (Weekly + Monthly Review, 2026-04-30 — period-aware `/review`, `generated_reviews` cache). iOS suite 471 / backend 243 / root tsc baseline 6 / backend tsc 0. **Pending (user-run):** 5b Tasks 14–15 (`PULSE_IMAP_ENCRYPTION_KEY` in `/opt/pulse/.env` + live droplet smoke), 5c Task 22 (push, regen JWT with `sync` scope, end-to-end live smoke). **Deferred (combined SP5-wide pass):** iPhone Expo Go visual verification for 5d–5g + web smoke; dev-client rebuild for `expo-notifications` (5e); live `/suggest-rituals` / `/nudge-today` / `/review` against real OpenRouter (gated on `OPENROUTER_API_KEY` carryover from SP2). |
